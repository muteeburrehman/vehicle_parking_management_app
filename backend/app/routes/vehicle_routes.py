...
async def add_vehicle_endpoint(
        dni: str,  # Get DNI from the URL path
    ...
    owner = db.query(Owners).filter(Owners.dni == dni).first()  # Use dni here instead of owner_id
    if owner is None:
        raise HTTPException(status_code=404, detail="Owner not found")

    # Store the uploaded document paths
    document_filenames = []
    for document in documents:
        file_location = f"vehicle_uploads/{document.filename}"
    ...
    vehicle_data = VehicleCreate(
        ...
        owner_id=owner.dni,  # Assuming you want the owner's ID, not DNI
    ...
    # Call the add_vehicle function to save vehicle data in the DB, passing document filenames
    new_vehicle = add_vehicle(db, vehicle_data, document_filenames)
...
def get_vehicle_endpoint(lisence_plate: str, db: Session = Depends(get_db)):
    ...
    vehicle_data = {
        ...
        'documents': vehicle.documents.split(',') if vehicle.documents else [],
...
async def handle_document_updates(vehicle, new_documents: List[UploadFile], remove_documents: List[str]):
    ...
    for document in new_documents:
        filename = f"{vehicle.lisence_plate}_{document.filename}"  # Prefix with DNI to avoid conflicts
        file_location = os.path.join(UPLOAD_DIR_VEHICLE, filename)
        try:
            with open(file_location, "wb") as file:
                content = await document.read()
                file.write(content)
            updated_documents.append(os.path.join(UPLOAD_DIR_VEHICLE, filename))
...
async def update_vehicle_endpoint(
    ...
    print(f"Updated vehicle documents: {vehicle.documents}")
